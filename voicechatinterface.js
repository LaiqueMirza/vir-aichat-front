...
const ChatInterface = () => {
	...
	useEffect(() => {
		if (agentId) {
			...
			const handleAudioChunk = (data) => {
				console.log('🎵 Received audio chunk:', data.audioData?.length || 0, 'bytes');
			...
			const handleMessageComplete = (data) => {
				...
				// Use the full response from the server event if available, otherwise use current streaming response
				const finalMessage = data.fullResponse || streamingResponse.trim();
				...
				// Clear streaming response and reset loading state
				setStreamingResponse('');
				setIsLoading(false);
			};
			...
			websocketService.on('responseStart', handleResponseStart);
			...
			return () => {
				...
				websocketService.off('responseStart', handleResponseStart);
	...
	}, [agentId, streamingResponse, isVoiceMode]);
	...
	const handleSendMessage = useCallback(
		...
		try {
			...
			if (isWebSocketConnected) {
				...
				// Set mic state to speaking if voice mode
				if (voiceMode) {
					setMicState('speaking');
				}
				
				// Send via WebSocket with socketId for backend correlation
				const success = websocketService.sendTextMessage(messageText, {
					agentId: agentId,
					leadId: sessionStorage.getItem(`lead_id_${agentId}`),
					voiceEnabled: voiceMode,
					sessionId: chatId || sessionStorage.getItem(`chat_id_${agentId}`)
				});
			...
			// **REST Fallback Path**
			console.log('📡 Using REST API for message (WebSocket not available)');
			const response = await chatAPI.sendMessage({
				message: messageText,
		...
		[agentId, chatId, isLoading, isVoiceMode, isWebSocketConnected]
	);
						...
						<div className="chat-messages">
							{isLoadingHistory ? (
								...
								<>
									...
									{streamingResponse && (
										<div className="message assistant message-assistant">
											<div className="message-avatar">
											...
											<div className="message-content">
												...
												<div className="message-time">
													...
													<span className="ml-1 text-blue-500">●</span> {/* Streaming indicator */}
												</div>
											</div>
										</div>
									)}
	...
	useEffect(() => {
		if (agentId) {
			...
			const handleTextChunk = (data) => {
				...
				// Process text chunk for TTS if in voice mode
				// Our enhanced StreamingTTSService now handles progressive word accumulation
				if (ttsService && isVoiceModeRef.current) {
					// Will accumulate text until it reaches the dynamic word limit
					// Starting with 5 words, then increasing by 2x each time
					ttsService.processTextChunk(data.chunk);
				}
			};

			const handleAudioChunk = (data) => {
				console.log(
					"🎵 Received audio chunk (deprecated - using native TTS instead):",
					data.audioData?.length || 0,
					"bytes"
				);
				// Audio chunks are now handled by native TTS instead of base64 audio
				// Set mic state to indicate audio is playing for compatibility
				if (isVoiceModeRef.current) {
					setMicState("speaking");
				}
			};
			...
			const handleMessageAudioChunk = async (data) => {
				...
				if (
					...
					console.log("� Processing text for TTS:", data.data.text);
					await ttsService.processTextChunk(data.data.text);
			...
			const handleMessageComplete = (data) => {
				...
				// Complete TTS streaming if in voice mode
				if (ttsService && isVoiceModeRef.current) {
					ttsService.processTextChunk("", true); // Signal completion
				}
			...
			const handleMessageChunkResponse = (data) => {
				...
				if (data.chunk || data.data?.response) {
					...
					if (ttsService && isVoiceModeRef.current) {
						ttsService.processTextChunk(chunk, data.isComplete);
					}
				}
			};
						...
						<button
							onClick={() => {
								...
								if (ttsService && ttsService.getState().isPlaying) {
									...
									ttsService.stop();
									// Reset mic state to idle when TTS is stopped
									setMicState("idle");
								}
								setShowMicPopover(false);
								handleVoiceToggle();
